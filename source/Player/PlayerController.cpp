...
namespace Player
{
	...
	void PlayerController::processBulletFire() {
		//sf::Vector2f currentPosition = player_model->getPlayerPosition();
		// Play bullet firing sound (optional)
		//if (player_model->canFire()) {
		// 
			// Get the bullet controller from ServiceLocator

		Bullet::BulletController* bulletController = Global::ServiceLocator::getInstance()->getBulletController();
		...
		// You may want to load the texture elsewhere and pass it to the sprite for efficiency
		sf::Sprite bulletSprite;  // Load the bullet sprite texture here
		
		// Create the bullet using BulletController
		bulletController->createBullet(bulletSpawnPosition, bulletVelocity, bulletSprite);
